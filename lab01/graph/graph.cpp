...
void Graph::checkVertex_(const Vertex& v) const {
	if (!vertices_.contains(v)) {
		throw std::invalid_argument("[Graph::checkVertex_] \"" + std::to_string(v) + "\" is not in V");
	}
}
...
Graph Graph::readFromFile(const std::filesystem::path& path) {
	...
	if (!stream) {
		throw std::invalid_argument("[Graph::readFromFile] " + path.string() + ": no such file");
	}
	stream >> graph;
	return graph;
}
...
void Graph::writeToFile(const std::filesystem::path& path) const {
	...
	if (!stream) {
		throw std::invalid_argument("[Graph::writeToFile] " + path.string() + ": can't open file");
	}
	stream << *this;
}
