...
class Tracing {
public:
	explicit Tracing(size_t& indent)
		: indent_{indent} {
		++indent;
	}
...
SearchResult depthFirstSearch(const HyperGraph& graph, Vertex from, const Vertices& to) {
	...
	const auto prove_rule = [&](size_t rule_id) {
		...
		if (wrong_rules.contains(rule_id)) {
			tracing << "Ранее было выяснено, что правило " << rule_id << " не доказуемо\n";
			return false;
		}

		if (closed_rules_fast_lookup.contains(rule_id)) {
			tracing << "Правило " <<  rule_id << " содержится в списке закрытых правил, значит оно доказано\n";
			return true;
		}
		tracing << "Правило " <<  rule_id << " не содержится в списке закрытых правил, доказываем входящие вершины\n";
		for (auto child : edges_lookup[rule_id]->to) {
			tracing << "Проверяем вершину " << child << '\n';
			if (!prove_node(child)) {
				tracing << "Так как вершина " << child << " не доказана, то правило " << rule_id << " не доказано\n";
				wrong_rules.insert(rule_id);
				return false;
			}
			tracing << "Вершина " << child << " доказана\n";
		}
		tracing << "Правило " <<  rule_id << " доказано, так как все входящие вершины доказаны, помещаем его в список закрытых правил\n";
	...
	prove_node = [&](Vertex v) {
		...
		if (closed_vertices_fast_lookup.contains(v)) {
			tracing << "Вершина " << v << " содержится в списке закрытых вершин, значит она доказана\n";
			return true;
		}
		tracing << "Вершина " << v << " не содержится в списке закрытых вершин, докажем какое-нибудь входящее правило\n";
