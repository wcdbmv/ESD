...
private:
	...
	// Список связей (множество согласований): ConstantName -> множество VariableName.
	...
	[[nodiscard]] UnresolvedLinkIt FindUnresolvedLinkFor(NameSV variable_name) {
		return std::find_if(unresolved_links.begin(), unresolved_links.end(), [variable_name](const auto& group) {
	...
	[[nodiscard]] bool UnifyVars(const Variable& var1, const Variable& var2) {
		...
		if (it1 == variables.end()) {
			...
			if (it2->second == nullptr) {
				std::cout << "Переменная " << var2 << " уже встречалась ранее, но на неё не распространена константа\n"
				          << "Добавим " << var1 << " к связи " << var2 << '\n';
				variables.emplace(var1.name, nullptr);
				FindUnresolvedLinkFor(var2.name)->emplace(var1.name);
				return true;
			}
			std::cout << "Распространим подстановку " << var2 << " = " << it2->second << " и на " << var1 << '\n';
		...
		if (it1->second == nullptr) {
			std::cout << "Переменная " << var1 << " уже встречалась ранее, но на неё не распространена константа\n";
			auto link1 = FindUnresolvedLinkFor(var1.name);
			if (it2 == variables.end()) {
				std::cout << "Переменная " << var2 << " встречается первый раз, добавим её к связи " << var1 << '\n';
				variables.emplace(var2.name, nullptr);
			...
			if (it2->second == nullptr) {
				...
				std::cout << "Переменные уже находятся в одной связи, ничего не делаем\n";
				return true;
			}
			std::cout << "У переменной " << var2 << " согласована подстановка " << it2->second << ", распространим её и на связь " << var1 << '\n';
	...
	[[nodiscard]] bool UnifyVarAndConst(const Variable& variable, const Constant& constant) {
		...
		if (it == variables.end()) {
			...
			std::cout << "Подстановка " << variable << " = " << GetValue(variable) << " не совпала с " << constant << '\n';
			return false;
		}
	...
	[[nodiscard]] static bool UnifyConsts(const Constant& const1, const Constant& const2) {
