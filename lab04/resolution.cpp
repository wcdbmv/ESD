...
void PrintInput(const std::vector<Clause>& axioms, const Clause& inverted_target) {
...
ResolutionResult FullResolution(const std::vector<Clause>& axioms, const Clause& inverted_target, size_t max_iterations) {
	std::cout << "[Полный перебор] ";
	PrintInput(axioms, inverted_target);

	auto clause_stack = Concatenate(axioms, {inverted_target});

	for (size_t iteration = 0, i = 0; i < clause_stack.size() - 1; ++i) {  // Проходим
		...
		for (size_t j = 0; j < clause_stack.size(); ++j) {             // по
			...
			if (res == ResolventResult::kEmptyClause) {                     // Если нашли пустой дизъюнкт,
				std::cout << "Найдена резольвента дизъюнктов " << clause1 << " и " << clause2 << ": {}\n";
				return ResolutionResult::kProvenByFoundingEmptyClause;  // то доказали утверждение.
			}
...
ResolutionResult BasicResolution(const std::vector<Clause>& axioms, const Clause& inverted_target, size_t max_iterations) {
	...
	for (size_t iteration = 0; /* nothing */; /* nothing */) {
		...
		for (const auto& clause2 : s2) {
			for (const auto& clause1 : axioms) {
				// Та же логика, что и в полном переборе, только добавляем не в общий стек, а в s2.
