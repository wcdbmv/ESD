...
std::unordered_map<std::string, bool> MakeClauseFastLookup(const AbsorbedClause& clause) {
...
/// Ищет имя атома из первого дизъюнкта, для которого существует атом, противоположный по знаку во втором дизъюнкте.
std::string FindOppositeAtomsName(const AbsorbedClause& first_clause,
	...
	for (auto&& atom : first_clause) {
		if (auto it = second_clause_fast_lookup.find(atom.name); it != second_clause_fast_lookup.end()) {
...
std::pair<Clause, ResolventResult> CreateResolvent(const Clause& first, const Clause& second) {
	auto first_absorbed_clause = first.Absorb();  // Приводим первый дизъюнкт к виду, где атомы не повторяются.
	auto second_absorbed_clause = second.Absorb();  // То же самое для второго дизъюнкта.
	...
	auto mb_opposite_atoms_name = FindOppositeAtomsName(first_absorbed_clause, second_absorbed_clause);
	...
	const auto append_resolvent_without_opposite_atom = [&mb_opposite_atoms_name, &resolvent](AbsorbedClause& absorbed_clause) {
