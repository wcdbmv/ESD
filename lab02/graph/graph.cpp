	...
	, edges_{std::move(edges)}
{
	...
	for (const auto& [from, tos, id] : edges_) {
		...
		if (ids.contains(id)) {
			throw std::invalid_argument("[HyperGraph::HyperGraph] Production rule with id \"" + std::to_string(id) + "\" already exists");
		}
		ids.insert(id);
	}
}
...
void HyperGraph::checkVertex_(const Vertex& v) const {
	if (!vertices_.contains(v)) {
		throw std::invalid_argument("[HyperGraph::checkVertex_] \"" + std::to_string(v) + "\" is not in V");
	}
}
...
HyperGraph HyperGraph::readFromFile(const std::filesystem::path& path) {
	...
	if (!stream) {
		throw std::invalid_argument("[HyperGraph::readFromFile] " + path.string() + ": no such file");
	}
	stream >> graph;
	return graph;
}
...
void HyperGraph::writeToFile(const std::filesystem::path& path) const {
	...
	if (!stream) {
		throw std::invalid_argument("[HyperGraph::writeToFile] " + path.string() + ": can't open file");
	}
	stream << *this;
}
