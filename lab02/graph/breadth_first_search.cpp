...
SearchResult breadthFirstSearch(const HyperGraph& graph, const Vertices& from, Vertex to) {
	...
	const auto all_closed = [&closed_vertices_fast_lookup](const Vertices& vs) {
		return std::all_of(vs.begin(), vs.end(), [&closed_vertices_fast_lookup](Vertex v) {
